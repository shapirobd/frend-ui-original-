...
const Grid = ({
  ...
  const spacingInPixels = spacing && spacing <= 5 && spacing > 0 ? spacing * 5 : 0;
  ...
  const [currBreakpoint, setCurrBreakpoint] = useState(getCurrBreakpoint(windowWidth, breakpoints, breakpointProps));
  console.log(currBreakpoint);
  const [largerBreakpoint, setLargerBreakpoint] = useState(getLargerBreakpoint(currBreakpoint));
  const [smallerBreakpoint, setSmallerBreakpoint] = useState(getSmallerBreakpoint(currBreakpoint));
  useEffect(() => {
    if (windowWidth <= breakpoints[smallerBreakpoint] && breakpointProps[smallerBreakpoint] || windowWidth >= breakpoints[largerBreakpoint] && breakpointProps[largerBreakpoint]) {
      setCurrBreakpoint(getCurrBreakpoint(windowWidth, breakpoints, breakpointProps));
    }
  }, [windowWidth]);
  useEffect(() => {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      width: `${windowWidth / breakpointProps[currBreakpoint] - spacingInPixels}px`,
